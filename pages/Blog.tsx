...
export const Blog: React.FC = () => {
  ...
  // Create an array of blog posts from CMS data, with fallback to static data
  const blogs = [
    cmsData.blog1 || blogPosts[0],
    cmsData.blog2 || blogPosts[1],
    cmsData.blog3 || blogPosts[2],
    cmsData.blog4 || blogPosts[3],
  ].filter(Boolean);
  ...
  return (
    <div className="px-6 py-20 max-w-7xl mx-auto min-h-screen">
      <div className="text-center max-w-3xl mx-auto mb-16">
        <ScrollReveal>
            <h1 className="text-4xl md:text-6xl font-bold mt-4 mb-6 text-neutral-900 dark:text-white">
              {cmsData.pageHeader?.mainHeading || 'Insights for Your Growth'}
            </h1>
            <p className="text-lg text-neutral-600 dark:text-neutral-400">
                {cmsData.pageHeader?.subHeading || 'Our perfect plan from our deep expertise in tech for your personal growth'}
            </p>
        </ScrollReveal>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {blogs.map((blog, i) => (
            <ScrollReveal key={i} delay={i * 100} className="group flex flex-col h-full">
                <NavLink to={`/blog/${blog?.slug || '#'}`} className="h-full">
                    <div className="bg-neutral-100 dark:bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-200 dark:border-neutral-800 hover:shadow-xl transition-all duration-300 h-full flex flex-col">
                        <div className="aspect-[16/10] overflow-hidden relative">
                            ...
                            <img src={blog?.image || 'https://images.unsplash.com/photo-1557804506-669a67965ba0?auto=format&fit=crop&q=80&w=1200'} alt={blog?.title || 'Blog post'} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        </div>
                        <div className="p-6 flex flex-col flex-grow">
                            <div className="flex items-center gap-4 text-xs text-neutral-500 mb-4">
                                <span className="flex items-center gap-1"><Calendar size={14} /> {blog?.date || 'No date'}</span>
                                <span>•</span>
                            ...
                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-200 dark:border-neutral-800">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-medium text-neutral-900 dark:text-white">{blog?.author || 'Anonymous'}</span>
                                </div>
